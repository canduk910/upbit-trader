...
class ExecHistoryStore:
    def __init__(self, path: Path = EXEC_HISTORY_DIR / "exec_history.json", max_entries: int = int(os.getenv("EXEC_HISTORY_MAX_ENTRIES", "1024"))):
        self.path = path
    ...
    def _save(self, data: List[Dict[str, Any]]) -> None:
        try:
            ...
            logger.warning("Failed to write exec history")
...
class BaseWebsocketListener(abc.ABC):
    def __init__(
        self,
    ...
    def _load_stats(self) -> None:
        ...
        self._stats_history.extend(history_entries[-self._stats_history_limit :])
    ...
    def _push_candle(self, ticker: str, timeframe: str, candle: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.warning(f"Redis candle push failed: {exc}")
    ...
    def _on_message(self, _, message: str) -> None:
        try:
            ...
            self.logger.warning(f"Failed to parse websocket message: {exc}")
            return
        if isinstance(payloads, list):
            for payload in payloads:
                self._process_payload(payload)
        elif isinstance(payloads, dict):
            self._process_payload(payloads)
    ...
    def _process_payload(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            success = False
            self.logger.warning(f"Websocket payload handling failed: {exc}")
        finally:
    ...
    def _on_open(self, ws: websocket.WebSocketApp) -> None:
        self._ws = ws
        try:
            ws.send(self.subscription_payload())
            self.logger.info("Websocket subscription sent")
        except Exception as exc:
            self.logger.warning(f"Failed to send websocket subscription: {exc}")

    def _on_error(self, _, error: Any) -> None:
        self.logger.warning(f"Websocket listener error: {error}")

    def _on_close(self, _, code, msg) -> None:
        self.logger.info(f"Websocket connection closed ({self._format_close_info(code, msg)})")
